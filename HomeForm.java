...
public class HomeForm extends JPanel {
    ...
    private JPanel createTopCardsPanel() {
        ...
        String[][] cards = {
            {"Total Students", "1,245", "INFO"},
            {"Total Staff", "87", "SUCCESS"},
            {"Courses", "24", "WARNING"},
            {"Dropout Rate", "2.18%", "DANGER"}
        };
    ...
    private JPanel createProgressItem(String name, int value) {
        ...
        progressBar.setForeground(Colors.CAVIAR);
        progressBar.setBackground(Colors.LIGHT_GRAY);
    ...
    private RoundPanel createQuickStatsPanel() {
        RoundPanel panel = new RoundPanel();
        panel.setBackground(Colors.LIGHT_GRAY);
        panel.setLayout(new BoxLayout(panel, BoxLayout.Y_AXIS));
        panel.setBorder(new EmptyBorder(20, 20, 20, 20));
        panel.setPreferredSize(new Dimension(0, 300));
